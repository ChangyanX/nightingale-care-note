...
import type { AppointmentRequest, CurrentUser, PatientDashboard, PatientSafeEntry } from "@/lib/api/types";
import { createSupabaseBrowserClient } from "@/lib/supabase/browser";

type PortalData = { user: CurrentUser; dashboard: PatientDashboard; token: string };
...
export default function PatientDashboardPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const dashboard = await apiGet<PatientDashboard>("/patient/dashboard", token);
        if (active) setData({ user, dashboard, token });
  ...
  async function askQuestion(event: FormEvent) {
    event.preventDefault(); if (!data) return; setError(null); setMessage(null);
    try {
      const result = await apiPost<{ entry: PatientSafeEntry; message: string }>("/patient/ai-question", data.token, { question });
      setData({ ...data, dashboard: { ...data.dashboard, history: [result.entry, ...data.dashboard.history] } });
